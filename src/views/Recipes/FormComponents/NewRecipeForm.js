...
import {lookupOptionIncludeToken, lookupOptionPOST} from '../../../helpers/functions_helpers';
...
class NewRecipeForm extends React.Component {
    ...
    handleSumbit = (event) =>{
        ...
        console.log('submit starts', this.state.items, this.state.fields)
        if (error_messages.length > 0) {
            ...
            const new_fields = Object.assign({}, fields, {owner:null})
            console.log('fetch recipe', fields)
            fetch(RECIPE_LIST_ENDPOINT, lookupOptionPOST(this.state.token, new_fields))
            .then(resp=> resp.json())
            .then(respData => {
                console.log('created recipe', respData)
                for(var i=0; i<items.length; i++){
                    ...
                    .then(resp=>resp.json())
                    .then(respData=>this.componentDidMount())
                }
            })
        }
    }
